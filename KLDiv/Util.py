...
def initDict(src, train=False):

	csvfile = open("test_doc_ids.csv", 'wb')
...
def fixMalletOutput(topicOutputPath, newPath, topic_num, testing = False):
	...
	for row in in_txt:
		...
		if testing:
			file_info = row[0].split(" ")[1].split("/")[-1]
		else:
